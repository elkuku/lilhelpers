 ...
 * @version   CVS: $Id: GetRequestDataSniff.php 8 2010-11-06 00:40:23Z elkuku $
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */

/**
 * Ensures that JRequest is used to access super globals.
 *
 * This applies to:
 * <b>$_REQUEST, $_GET, $_POST</b> and <b>$_FILES</b>
 *
 * <b>DISABLED !</b>
 *
 * <b class="bad">$id = $_GET['id'];</b>
 * <b class="good">$id = JRequest::getInt('id');</b>
 *
 * @category  PHP
 * @package   PHP_CodeSniffer_MySource
 * @author    Greg Sherwood <gsherwood@squiz.net>
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class KuKu_Sniffs_Joomla_GetRequestDataSniff implements PHP_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $error = 'The %s super global must not be accessed directly; use JRequest::getXXX(';
        $data  = array($varName);

        if($usedVar !== '')
        {
            $error = sprintf($error.'%s) instead'
            , $varName, $usedVar);
            $type  .= 'WithVar';
            //            $error .= '%s, \'%s\'';
            //            $data[] = $usedVar;
        ...
        {
            $error = sprintf($error.') instead'
            , $varName);
        }

        //        $error .= ') instead';
        $phpcsFile->addError($error, $stackPtr, 'SuperglobalAccessed');
    }//function
}//class
